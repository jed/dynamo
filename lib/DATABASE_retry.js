...
Database.prototype = {
  ...
  request: function(target, data, cb) {
    ...
    !function request(retries) {
      var req = new Request(self.host, target, data)

      self.account.sign(req, function(err, req) {
        if (err) cb(err)

        else req.send(function(err, data) {
          if (err) {
            if (retries > 3) return cb(err, data)

            if (err.statusCode == 500 || err.statusCode == 503) {
              return setTimeout(request, Math.pow(4, retries) * 100, retries + 1)
            }
